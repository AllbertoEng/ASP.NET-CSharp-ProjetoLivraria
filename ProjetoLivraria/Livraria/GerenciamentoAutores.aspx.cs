...
namespace ProjetoLivraria.Livraria
{
    public partial class GerenciamentoAutores : System.Web.UI.Page
    {
        ...
        protected void BtnNovoAutor_Click(object sender, EventArgs e)
        {
            try
            {
                decimal ldcIdAutor = this.ListaAutores.OrderByDescending(autor => autor.aut_id_autor).First().aut_id_autor + 1;
        ...
        protected void gvGerenciamentoAutores_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    HttpContext.Current.Response.Write("<script>alert('Erro ao atualizar cadastro');</script>");
                }
            }
        }
        ...
        protected void gvGerenciamentoAutores_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                ...
                if(loAutor != null)
                {
                    ...
                    {
                        HttpContext.Current.Response.Write("<script>alert('Não é possível remover o autor pois tem livros associados a ele.');</script>");
                    }
                    else
                    {
                        this.ioAutoresDAO.RemoverAutor(loAutor);
                        this.CarregaDados();
            ...
            {
                HttpContext.Current.Response.Write("<script>alert('Erro ao remover autor selecionado.');</script>");
            }
        }
